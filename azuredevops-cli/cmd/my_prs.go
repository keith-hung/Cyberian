...
// RunMyPRs lists pull requests where the authenticated user is a creator or reviewer,
// across all projects in the collection.
func RunMyPRs(gf *GlobalFlags, args []string) {
	status := "active"
	role := "all"
	project := gf.Project // optional filter
	...
	for _, e := range prMap {
		...
		if project != "" && !strings.EqualFold(pr.Repository.Project.Name, project) {
			continue
		}
