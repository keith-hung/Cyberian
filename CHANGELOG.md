...
- 重新發佈,讓 `change-password` skill 能被已安裝的 plugin 取得。它是在 `0.3.0` tag 之後才併入 `main`、且沿用同一版號,導致客戶端因「版號未變」而不會重抓;bump 到 `0.3.1` 讓 Claude Code 偵測到更新並抓下該 skill。各 CLI 無功能變更。
