...
> **關於 read-email：** 它是本 marketplace 內的**獨立 plugin**（原始碼於 [keith-hung/claude-read-email](https://github.com/keith-hung/claude-read-email)），與上方四個內建 skill 不同，需**另外安裝**：
>
> ```bash
> claude plugin install read-email@cyberian-marketplace
> ```
>
> 執行需求：[uv](https://docs.astral.sh/uv/) 與 Python 3.10+。其餘 skill 的 CLI binary 會自動下載，read-email 則透過 uv 執行 Python 解析器，因此不需環境變數但需要 uv。
...
> **已退役：** 此 CLI 對應的 skill 已自 plugin 移除、不再隨 release 發布，Claude Code 不會再觸發它。原始碼保留於 `timecard-cli/` 供參考與必要時手動建置，工時填報請改用下方的 nouveau-timecard-cli。

透過 web scraping 操作舊版工時系統，支援 cookie-based session 自動管理。
